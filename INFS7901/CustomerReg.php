...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["firstname"] || $_POST["lastname"]))){
      ...
      if (empty(trim($_POST["email"] || $_POST["mobile"] || $_POST["address"]))){
      $blank = "Please don't leave any field empty";
    }
    else{
        // Prepare a select statement
        $sql = "SELECT Firstname, Lastname FROM customerdetails WHERE Firstname = ? AND Lastname = ? ";

        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt,  "ss", $param_first, $param_last);

            // Set parameters
            $param_first = trim($_POST["firstname"]);
            $param_last == trim($_POST["lastname"]);
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err) && empty($blank)){
        // Prepare an insert statement
        $sql = "INSERT INTO customerdetails (FirstName,LastName,Email,Mobile,Address,Pass) VALUES (?, ?, ?, ?, ?, ?)";

        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssss", $param_firstname, $_POST["lastname"], $param_email, $param_mobile, $param_address, $param_password);

            // Set parameters
            $param_firstname = $param_first;
    ...
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <div class="form-group">
                ...
                <span class="invalid-feedback"><?php echo $blank; ?></span>
            </div>
            <div class="form-group">
                <label>Mobile</label>
                <input type="number" name="mobile" class="form-control <?php echo (!empty($username_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $username; ?>">
                <span class="invalid-feedback"><?php echo $blank; ?></span>
            </div>
            <div class="form-group">
                <label>Address</label>
                <input type="text" name="address" class="form-control <?php echo (!empty($username_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $username; ?>">
                <span class="invalid-feedback"><?php echo $blank; ?></span>
            </div>
            <div class="form-group">
